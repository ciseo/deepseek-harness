...
describe('approveEscalation', () => {
  ...
  it('a non-widening request returns the current effective mode without asking', async () => {
    const seen: unknown[] = []
    ...
    await expect(approveEscalation(req({ requestedMode: 'workspace-write', effectiveMode: 'danger-full-access' as never }), spy)).resolves.toBe('danger-full-access')
    expect(seen).toEqual([])
  })
